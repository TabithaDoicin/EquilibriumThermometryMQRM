...
def ExpectedSVal(Dg,De):
    \"\"\"Return the expected largest singular value for a (Dg×De) complex Ginibre matrix.

    Used to set the coupling prefactor in the heatmap generator.
    \"\"\"
...
def mode_eigs_wishart(Dg, De, normalised, beta=2, c=1.0):
    \"\"\"Approximate mode of Wishart eigenvalues via Laguerre polynomial roots.

    If *normalised* is True, eigenvalues are divided by the largest one.
    \"\"\"
    M = np.min([Dg,De])
    N = np.max([Dg,De])
...
def brightbrightsector():
    Xq = mode_eigs_wishart(Dg, De, Individuallynormalised)
    svddiagonals = [x**0.5 for x in Xq]
    brightbrightqfi = generate_qfi_list_brightpeak_fast(wc, wa, Xq, Tlist, Dg,De,Dmin=0, Dplu=0, Dk=0, gprefactor=gprefactor,Ocutoff=theta)
